...
// ステップIDの配列（順序保証）
export const OIDC_STEP_ORDER: number[] = [1, 2, 3, 4, 5, 6, 7, 8, 9];

// ステップラベル一覧（StepIndicator用）
export const OIDC_STEP_LABELS: string[] = OIDC_STEP_ORDER.map(
    (id) => (OIDC_STEPS[id] as FlowStep).label
);
